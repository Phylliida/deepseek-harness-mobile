...
export const SERVICE_API: readonly ServiceApiEntry[] = [
  ...
  {
    key: 'memory',
    summary: 'Abstract permanent-memory service; load one implementation per context as `ctx.memory`.',
    ...
    methods: [
      {
        signature: 'abstract run(command: string): Promise<string>',
        description: 'Run one memory command.',
        parameters: [{ name: 'command', description: 'one command in the OptMem grammar, e.g. `wake`, `wake 2 296`, `note "one line"`, `nap 0-1 "summary"`, `recall foo|bar`, `zoom 16-31`, `forget 16-31`. An empty or whitespace command returns the usage text.' }],
        returns: 'the command\'s output text.',
        throws: ['{MemoryError} when the command is malformed or names something the store does not hold.'],
      },
    ],
  },
  {
