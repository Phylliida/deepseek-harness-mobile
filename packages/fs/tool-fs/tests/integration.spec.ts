...
// --------------------------------------------------------------------------
// LAX deployment: the policy gate with allowUnobservedMutation — unseen targets
// fall through to the bare provider, observed targets keep CAS.
// --------------------------------------------------------------------------
describe('lax policy (fs-observation-policy with allowUnobservedMutation)', () => {
  beforeEach(async () => {
    dir = await mkdtemp(join(tmpdir(), 'dsh-tool-fs-lax-'))
    ctx = new Context()
    await ctx.plugin(SystemPrompt)
    await ctx.plugin(ToolRuntime)
    await ctx.plugin(LocalFileSystem, { cwd: dir })
  ...
  it('write OVERWRITES an existing unread file', async () => {
    await writeFile(join(dir, 'a.txt'), 'original')
    const result = await call('write', { file_path: 'a.txt', content: 'clobbered' })
    expect(result.isError).toBe(false)
    expect(await readFile(join(dir, 'a.txt'), 'utf8')).toBe('clobbered')
  ...
  it('edit edits an UNREAD existing file', async () => {
    await writeFile(join(dir, 'a.txt'), 'hello world')
    const result = await call('edit', { file_path: 'a.txt', old_string: 'world', new_string: 'there' })
    expect(result.isError).toBe(false)
    expect(await readFile(join(dir, 'a.txt'), 'utf8')).toBe('hello there')
  ...
  it('an observed target still rejects a stale write (CAS survives lax mode)', async () => {
    await writeFile(join(dir, 'a.txt'), 'original')
    expect((await call('read', { file_path: 'a.txt' })).isError).toBe(false)
    await writeFile(join(dir, 'a.txt'), 'changed-externally') // out-of-band change
    const result = await call('write', { file_path: 'a.txt', content: 'replaced' })
    expect(result.isError).toBe(true)
    expect(result.error).toMatchObject({ info: { code: 'FS_STALE_VERSION' } })
    expect(await readFile(join(dir, 'a.txt'), 'utf8')).toBe('changed-externally')
  })

  it('editing a target observed ABSENT still reports FS_NOT_FOUND', async () => {
    // Reading a missing target records absence...
    expect((await call('read', { file_path: 'missing.txt' })).isError).toBe(true)
    // ...and a later edit stays rejected even though the mode is lax.
    const result = await call('edit', { file_path: 'missing.txt', old_string: 'a', new_string: 'b' })
    expect(result.isError).toBe(true)
