...
describe('lax mode: allowUnobservedMutation relaxes only the unseen case', () => {
  async function setupLax() {
    const ctx = new Context()
    await ctx.plugin(FsPolicy, { allowUnobservedMutation: true })
    return ctx
  }

  it('an unseen write falls through to the bare provider (no intent)', async () => {
    const ctx = await setupLax()
    expect(await writeIntent(ctx, target('a.txt'), ownerExec({}))).toBeUndefined()
  })

  it('a no-owner write falls through (no intent)', async () => {
    const ctx = await setupLax()
    expect(await writeIntent(ctx, target('a.txt'), undefined)).toBeUndefined()
  })

  it('an unseen edit proceeds with no version basis (undefined)', async () => {
    const ctx = await setupLax()
    expect(await editIntent(ctx, target('a.txt'), ownerExec({}))).toBeUndefined()
  })

  it('a no-owner edit proceeds with no version basis (undefined)', async () => {
    const ctx = await setupLax()
    expect(await editIntent(ctx, target('a.txt'), undefined)).toBeUndefined()
  })

  it('an observed-present target still CAS-guards both write and edit', async () => {
    const ctx = await setupLax()
    const exec = ownerExec({})
    ctx.emit('fs/observed', target('a.txt'), present('v9'), exec)
    expect(await writeIntent(ctx, target('a.txt'), exec)).toEqual({ kind: 'replaceIfVersion', version: 'v9' })
    expect(await editIntent(ctx, target('a.txt'), exec)).toEqual({ version: 'v9' })
  })

  it('an observed-absent target keeps createIfAbsent and FS_NOT_FOUND', async () => {
    const ctx = await setupLax()
    const exec = ownerExec({})
    ctx.emit('fs/observed', target('a.txt'), absent, exec)
    expect(await writeIntent(ctx, target('a.txt'), exec)).toEqual({ kind: 'createIfAbsent' })
