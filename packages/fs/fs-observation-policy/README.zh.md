...
`allowUnobservedMutation: true` 仅放宽**未见**目标的决策：所有者从未观测过的目标的写入会落到裸提供方的无条件写入，而不是 `createIfAbsent`（因此覆盖未观测的现有文件不再因 `FS_NOT_OBSERVED` 失败）；未见目标的编辑也会在没有版本基准的情况下继续，而不是以 `FS_NOT_OBSERVED` 拒绝。已确认的观测仍然参与决策：已观测为存在的目标保留其 CAS 防护（观测后发生的外部变更仍然以 `FS_STALE_VERSION` 失败），已观测为缺失的目标保留 `createIfAbsent` / `FS_NOT_FOUND`。随附的 bundle 保持默认值；仅在能够接受盲目覆盖部署从未读取过的文件时才启用该选项。
