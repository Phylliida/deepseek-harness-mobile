...
export const Config: z<Config> = z.object({
  allowUnobservedMutation: z.boolean().default(false),
})
...
class ObservedStateGate {
  /**
   * @param allowUnobservedMutation - see {@link Config.allowUnobservedMutation};
   *   relaxes only the unseen-target write/edit decisions.
   */
  constructor(private readonly allowUnobservedMutation: boolean) {}
  ...
  /**
   * Decide the write intent: confirmed absent ⇒ `createIfAbsent`; confirmed
   * present ⇒ `replaceIfVersion` at the observed version; unseen ⇒
   * `createIfAbsent` by default so an existing unobserved target fails closed,
   * or no guard (unconditional provider write) when
   * `allowUnobservedMutation` is set.
   */
  writeIntent(target: FsTarget, actor: object | undefined): FsWriteIntent | undefined {
    ...
    if (prior === undefined) {
      return this.allowUnobservedMutation ? undefined : { kind: 'createIfAbsent' }
    }
  ...
  /**
   * Decide the edit version guard: unseen rejects with `FS_NOT_OBSERVED` by
   * default, or proceeds without a version basis (unconditional provider edit)
   * when `allowUnobservedMutation` is set; confirmed absence rejects with
   * `FS_NOT_FOUND`; presence supplies the observed version as the CAS basis.
   */
  editIntent(target: FsTarget, actor: object | undefined): { version: FsVersion } | undefined {
    const owner = this.owner(actor)
    const prior = owner ? this.get(owner, target.targetKey) : undefined
    if (!owner || prior === undefined) {
      if (this.allowUnobservedMutation) return undefined
 ...
 * @param config - validated {@link Config}; the schemastery defaults are already applied.
 */
export function apply(ctx: Context, config: Config): void {
  // schemastery's .default() guarantees the field is set after validation.
  const gate = new ObservedStateGate(config.allowUnobservedMutation as boolean)
