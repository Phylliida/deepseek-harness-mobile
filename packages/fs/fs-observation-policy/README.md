...
- id: fs-observation-policy
  ...
  config:
    allowUnobservedMutation: false  # default; true relaxes the unseen-target decisions
```

`allowUnobservedMutation: true` relaxes only the **unseen**-target decisions: a write whose target this owner has never observed falls through to the bare provider's unconditional write instead of `createIfAbsent` (so overwriting an unobserved existing file no longer fails `FS_NOT_OBSERVED`), and an unseen edit proceeds without a version basis instead of rejecting `FS_NOT_OBSERVED`. Confirmed observations still decide: an observed-present target keeps its CAS guard (an external change since the observation still fails `FS_STALE_VERSION`), and an observed-absent target keeps `createIfAbsent` / `FS_NOT_FOUND`. The shipped bundles keep the default; enable this only where blind overwrites of files the deployment has never read are acceptable.
