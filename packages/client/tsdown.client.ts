 ...
 * (external) or a leak the purity gate rejects. `dsh-coding-activity/document`
 * qualifies as a pure, dependency-free math module — the activity log's
 * canonical merge the browser applies to its own pending stamps.
 */
export const INLINE_SAFE = /^@deepseek-ai\/(?:dsh-(host-apiproxy|session|llm|tools|brand)(\/|$)|dsh-coding-activity\/document$)/
