...
- `CHANNEL_LEAK_CODE` — the provider-neutral code for the sibling failure: a completed `stop` whose visible reply vanished into the reasoning channel (a gateway misparsing the model's channel markup), reported only by an adapter the deployment warned about such markup. Sharing `EMPTY_RESPONSE`'s rationale — nothing durable, safe to repeat — it joins the default retryable set.
