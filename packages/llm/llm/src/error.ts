...
/**
 * Canonical provider-neutral code for a completed response whose visible reply
 * vanished into the reasoning channel. Gateway-style providers re-parse a
 * model's channel markup into structured thinking/text/tool blocks; when that
 * parse loses a boundary, the reply arrives as a silent reasoning-only stop
 * whose reasoning text still contains the markup the gateway failed to
 * recognize. Only an adapter the deployment warned about such markup (its
 * configured leak markers) reports this. As with
 * {@link EMPTY_RESPONSE_CODE}, the attempt produced nothing durable, so retry
 * policy treats it as safe to repeat.
 */
export const CHANNEL_LEAK_CODE = 'CHANNEL_LEAK'
