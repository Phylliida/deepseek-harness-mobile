...
import { CHANNEL_LEAK_CODE, EMPTY_RESPONSE_CODE } from './error.ts'

// The default budget rides out multi-second connection blips: twelve retries
// with 1s→2s→4s→8s→15s capped waits sum to roughly two minutes of delay
// before a request step gives up.
...
export interface BackoffConfig {
  /** Initial local exponential-backoff delay in milliseconds (default 1000). */
  initialDelayMs?: number
  /** Maximum locally scheduled or accepted provider delay in milliseconds (default 15000). */
  maxDelayMs?: number
...
export interface NormalRetryPolicyConfig {
  ...
  mode: 'normal'
  /** Maximum eligible retries after the first request (default 12). */
  maxRetries?: number
