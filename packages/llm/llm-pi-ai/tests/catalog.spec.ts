...
describe('hand-declared providers', () => {
  ...
  it('carries a model’s channel-leak markers into the resolved profile', () => {
    const resolved = resolveProfiles({
      'acme-gateway': {
        api: 'openai-completions',
        baseURL: 'https://acme.test',
        models: [
          { id: 'bare' },
          { id: 'kimi', channelLeakMarkers: ['<|open|>', '<|close|>', '<|sep|>'] },
        ],
      },
    })
    const markers = resolved.get('acme-gateway')?.channelLeakMarkers

    expect(markers?.get('bare')).toBeUndefined()
    expect(markers?.get('kimi')).toEqual(['<|open|>', '<|close|>', '<|sep|>'])
  })

  it('rejects an empty channel-leak marker naming the model', () => {
    expect(() => resolveProfiles({
      'acme-gateway': {
        api: 'openai-completions',
        baseURL: 'https://acme.test',
        models: [{ id: 'kimi', channelLeakMarkers: [''] }],
      },
    })).toThrow(/provider "acme-gateway" model "kimi" channelLeakMarkers must contain only non-empty strings/)
  })
