...
import { createUserMessage, CallId, CHANNEL_LEAK_CODE, CONTEXT_WINDOW_EXCEEDED_CODE, EMPTY_RESPONSE_CODE, LlmError, createMessage } from '@deepseek-ai/dsh-llm'
...
describe('toStreamChunks', () => {
  ...
  it('surfaces a marker-tainted thinking-only done as a retryable CHANNEL_LEAK error', async () => {
    const collapsed = assistant({
      stopReason: 'stop',
      content: [{ type: 'thinking', thinking: 'plan <|open|>response<|open|>tools<|open|>call' }],
    })
    const chunks = await collect(toStreamChunks(feed(
      { type: 'done', reason: 'stop', message: collapsed },
    ), undefined, ['<|open|>']))
    const terminal = chunks.at(-1)
    if (terminal?.type !== 'finish') throw new Error('expected a finish chunk')
    expect(terminal.reason).toEqual({
      kind: 'error',
      failure: {
        message: 'model "deepseek-v4-flash" reply collapsed into the reasoning channel (found "<|open|>")',
...
describe('mapStopReason / mapUsage', () => {
  ...
  it('classifies a marker-tainted thinking-only stop as a CHANNEL_LEAK error', () => {
    expect(mapStopReason(
      assistant({ stopReason: 'stop', content: [{ type: 'thinking', thinking: 'hmm <|open|>response answer <|open|>tools' }] }),
      undefined,
      ['<|open|>', '<|close|>'],
    )).toEqual({
      kind: 'error',
  ...
  it('keeps a marker-free thinking-only stop successful even with markers configured', () => {
    expect(mapStopReason(
      assistant({ stopReason: 'stop', content: [{ type: 'thinking', thinking: 'perfectly ordinary reasoning' }] }),
      undefined,
      ['<|open|>'],
    )).toEqual({ kind: 'stop' })
  })

  it('keeps a stop successful when markers appear in reasoning beside a visible reply', () => {
    expect(mapStopReason(
      assistant({
        stopReason: 'stop',
        content: [
          { type: 'thinking', thinking: 'pondering <|open|>syntax' },
