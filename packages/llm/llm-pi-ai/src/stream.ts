...
import { CallId, CHANNEL_LEAK_CODE, CONTEXT_WINDOW_EXCEEDED_CODE, EMPTY_RESPONSE_CODE, isContextWindowExceededError, isQuotaExceededError, LlmError, QUOTA_EXCEEDED_CODE } from '@deepseek-ai/dsh-llm'
 ...
 * @param channelLeakMarkers - deployment-declared markup strings that must
 *   never appear in reasoning text (see
 *   {@link PiAiModelProfile.channelLeakMarkers}). A `stop` arriving with only
 *   thinking blocks whose joined text contains one maps to a retryable
 *   `CHANNEL_LEAK` error instead of silently closing the turn with nothing
 *   visible.
...
export function mapStopReason(
  ...
  channelLeakMarkers?: readonly string[],
): FinishReason {
  ...
  switch (message.stopReason) {
    case 'stop':
      ...
      // A thinking-only stop would otherwise end the turn with nothing visible
      // or durable. With deployment-declared leak markers the reasoning text
      // itself proves the gateway folded the reply into the thinking channel;
      // retry rather than surface a silent turn.
      if (channelLeakMarkers !== undefined && channelLeakMarkers.length > 0
        && message.content.every(block => block.type === 'thinking')) {
        const reasoning = message.content
          .map(block => (block as { type: 'thinking'; thinking: string }).thinking)
          .join('')
        const leaked = channelLeakMarkers.find(marker => reasoning.includes(marker))
        if (leaked !== undefined) {
          return {
            kind: 'error',
            failure: {
              message: `model "${message.model}" reply collapsed into the reasoning channel (found "${leaked}")`,
              code: CHANNEL_LEAK_CODE,
            },
          }
        }
      }
 ...
 * @param contextWindow - resolved catalog capacity for usage-based overflow detection.
 * @param channelLeakMarkers - forwarded to {@link mapStopReason}; deployment-declared
 *   markup strings that make a reasoning-only `stop` a retryable CHANNEL_LEAK error.
  ...
  for await (const event of events) {
    switch (event.type) {
      ...
      case 'done':
        ...
        yield {
          type: 'finish',
          reason: mapStopReason(event.message, contextWindow, channelLeakMarkers),
      ...
      case 'error':
        ...
        yield { type: 'finish', reason: mapStopReason(event.error, contextWindow, channelLeakMarkers) }
        return
