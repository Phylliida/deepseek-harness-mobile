...
export interface RouteCatalog {
  ...
  /**
   * Channel-leak markers explicitly configured per model id. Kept out of the
   * pi-ai `Model` object for the same layered reason as
   * {@link configuredMaxTokens}: the marker set is a deployment policy the
   * adapter enforces at stream-finalization, not a fact pi-ai's provider
   * understands.
   */
  channelLeakMarkers: ReadonlyMap<string, readonly string[]>
}
...
export function resolveRouteModels(request: RouteCatalogRequest): RouteCatalog {
  ...
  const channelLeakMarkers = new Map<string, readonly string[]>()
  const models = entries.map((entry) => {
    ...
    if (entry.channelLeakMarkers !== undefined) {
      if (entry.channelLeakMarkers.some(marker => marker.length === 0)) {
        invalid(provider, `model "${entry.id}" channelLeakMarkers must contain only non-empty strings`)
      }
      channelLeakMarkers.set(entry.id, Object.freeze([...entry.channelLeakMarkers]))
    }
    return {
  ...
  return { models, configuredMaxTokens, channelLeakMarkers }
}
