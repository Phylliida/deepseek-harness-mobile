...
export interface ResolvedPiAiProviderProfile
  ...
  /**
   * Per-model channel-leak markers from the resolved catalog
   * ({@link PiAiModelProfile.channelLeakMarkers}); the adapter enforces them
   * when a `stop` arrives carrying only reasoning blocks.
   */
...
const modelFields = {
  ...
  channelLeakMarkers: z.array(z.string()),
}
...
export function resolveProfiles(
  ...
  for (const [provider, source] of entries) {
    ...
    resolved.set(provider, {
      ...
      channelLeakMarkers: catalog.channelLeakMarkers,
