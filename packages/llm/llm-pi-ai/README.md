...
### Channel-leak markers

Some gateways do not receive a model's structured thinking/text/tool blocks from upstream; they re-parse the model's raw channel grammar (Kimi's `<|open|>response<|sep|>…` markup and its kin) into those blocks themselves. When that parse loses a boundary, the whole visible reply — including any tool calls — folds into one thinking block and the stream ends as a silent reasoning-only `stop`: the turn closes with nothing for the user or the loop to act on. `channelLeakMarkers` on a `models` entry (or `modelOverrides` value) is the deployment declaring which markup strings must never appear in *reasoning* text for that model. A `stop` whose content is thinking-only and whose joined reasoning contains a configured marker then maps to a retryable `CHANNEL_LEAK` error finish — retryable by default, like `EMPTY_RESPONSE` — so the failed step retries instead of stalling the turn on invisible output; an otherwise ordinary response (markers in reasoning beside a visible reply) is left alone. Like everything else on the entry, only explicit configuration opts a model in.
...
- OpenRouter-compatible endpoints report the actual billed amount on the response usage object. The harness's pnpm patch on pi-ai (`patches/@earendil-works__pi-ai@0.82.1.patch`) preserves it as `Usage.providerCost`, and `mapUsage` carries it to `TokenUsage.costUsd` so rate-based cost estimates never price that call again; pi-ai's own catalog-estimated `usage.cost` block is not read.
