...
### 通道泄漏标记（channelLeakMarkers）

有些网关并不会从上游直接拿到模型结构化的 thinking/text/tool 块；它们自行把模型的原生通道文法（Kimi 的 `<|open|>response<|sep|>…` 标记及其同类）重新解析成这些块。当这次解析丢掉一个边界时，整条可见回复——包括工具调用——会折叠进一个 thinking 块，并以一个静默的仅推理 `stop` 终止：回合就此结束，没有任何可供用户或主循环继续的内容。`models` 条目（或 `modelOverrides` 值）上的 `channelLeakMarkers`，就是部署方声明该模型的*推理*文本中绝不应出现的标记字符串。此后，一个内容仅含 thinking 且拼接后的推理文本含有已配置标记的 `stop`，会映射为可重试的 `CHANNEL_LEAK` 错误 finish——与 `EMPTY_RESPONSE` 一样默认可重试——于是失败的那一步会自动重试，而不是让整个回合卡在不可见的输出上；形态正常的响应（推理里出现标记但旁边还有可见回复）不受影响。与条目上的其他字段一样，只有显式配置才会让某个模型启用该检测。
...
- **重试策略由提供方持有，而不是 SDK 重试**：每个提供方 profile 都可以配置嵌套的 `retryPolicy`，由 `dsh-llm-retry` 在 agent 的失败步骤扩展点上执行；pi-ai SDK 重试仍保持禁用，因此持久化的 agent 步骤与 `llm/retry` 事件记录每次可见尝试，直接 `ctx.llm.stream()` 调用仍只尝试一次。默认策略会重试 `TRANSPORT`，而被中断的响应携带的也正是该 code：流式输出中途断开、在终止事件之前被截断的流，以及映射到默认策略的 OpenRouter 规范 `error` finish reason。
