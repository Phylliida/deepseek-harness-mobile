...
`tokenUsage` carries the complete durable log's `uncachedInputTokens`, `outputTokens`, `cacheReadTokens`, and `cacheWriteTokens`. Usage chunks are counted even when a request later fails; a final assistant-message usage for the same `(turn, step)` replaces that sample instead of double-counting it. Reasoning remains an output subdivision. The single last-sample slot relies on a session-log ordering property: once a later step reports usage, a legal log never reports usage for an earlier step again. Provider-reported billed cost folds alongside with the same replace semantics: samples carrying `TokenUsage.costUsd` (OpenRouter reports one per request) sum into the optional `reportedCostUsd`, and their tokens are excluded from the optional `unratedTokens` bucket set, so rate-based estimates price only usage no provider billed. Both fields stay absent while no sample reports a cost — a reported `$0` still counts as billed fact.
