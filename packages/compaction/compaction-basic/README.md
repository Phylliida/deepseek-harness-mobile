...
| `maxTokens` | no (default `8192`) | Provider generation cap for the summarization call; may include reasoning tokens. `0` omits the cap from the request so the provider applies the model's own maximum. |
