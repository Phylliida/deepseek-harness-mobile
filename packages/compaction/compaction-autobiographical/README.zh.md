...
| `contextWindowTokensByModel` | 否（默认：空） | 按会话路由模型键控的单模型运行上限；优先于 65536 默认上限，但低于显式配置的 `contextWindowTokens`。 |
...
每条回忆都要支付一次受 `maxTokens` 限制的推理——每个被压缩的区块一次，每次合并到更粗层级再一次——其输入是已镜像的历史加上上述框定内容。压缩以串行 tick 每次处理一个区块；每个形成记忆的 tick 追加一条携带新铸回忆的 `autobio/memory` 事件，聊天界面按记忆形成调用各渲染一条状态行：调用进行中实时流出文本（`autobio/memory-progress` 冲刷），随后沉淀为新铸的回忆，点击可展开查看。当表层仍在预算内时 tick 在后台涓流推进；但当选择器找不到可行的布局时，轮次会等待：追赶 tick 在推理线程上运行，直到布局可行——一个 tick 没有形成任何新记忆（没有可压缩内容）即结束等待。即便如此，金字塔尚在构建中期（浅层等待合并组包）时，完全折叠后的下限仍可能超过预算，因此本次通过会在测得的下限处重试一次，并在那里落地最佳布局，而不是让会话保持原始状态搁浅；只有当重试也得不出布局时，本次通过才保持表层不变。
