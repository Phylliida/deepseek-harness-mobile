...
- `scrubbedParentEnv()` / `SENSITIVE_ENV_PATTERN` 是唯一一份共享的环境清理定义：环境中形似凭据的名称与 `DSH_*` 名称都会被丢弃，显式 `env` 在清除之后合并。该清理还会修复标准临时目录变量（`TMPDIR`／`TMP`／`TEMP`／`TEMPDIR`）：指向已不存在目录的变量——例如死掉的 `nix-shell` 场景，长期存活的 harness 会持续携带已被删除的临时目录——会被改写为平台默认目录；而指向已存在目录的变量（操作方有意配置的自定义临时根目录）则原样保留。本地的普通 spawn 与终端 spawn 都应用该定义；拥有自身 spawn 的 SDK 管理传输可直接导入它。
