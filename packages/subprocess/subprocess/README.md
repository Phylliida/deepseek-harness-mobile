...
- `scrubbedParentEnv()` / `SENSITIVE_ENV_PATTERN` are the one shared scrub definition: ambient credential-shaped and `DSH_*` names are dropped, and explicit `env` merges after the scrub. The scrub also repairs the standard temp variables (`TMPDIR`/`TMP`/`TEMP`/`TEMPDIR`): a variable naming a missing directory — the ambient dead-`nix-shell` case, where a long-lived harness keeps the deleted scratch dir in its env — is repointed to the platform default, while an existing directory (an operator's deliberate custom temp root) is honored verbatim. The local ordinary and terminal spawns both apply it; SDK-managed transports that own their spawn may import it directly.
