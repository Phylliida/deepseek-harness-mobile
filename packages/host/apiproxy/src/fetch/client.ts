 ...
 * `crypto.randomUUID()` exists only on HTTPS/loopback origins, which would
 * break every RPC on a plain-HTTP LAN deployment; `crypto.getRandomValues()`
 * is exposed on insecure origins (and Node ≥ 15), keeping this base
 * platform-neutral.
