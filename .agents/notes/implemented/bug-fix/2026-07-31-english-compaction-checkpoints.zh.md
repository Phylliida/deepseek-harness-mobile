...
`COMPACTION_INSTRUCTION` 要求生成英语的内部工程检查点。它要求模型在必要时翻译叙述性源材料，同时保留精确的字面量；这包括路径、命令、错误、标识符、签名，以及精确性重要时的引用措辞。检查点的标题仍沿用既有的结构化格式，[以全面细节为优先](2026-09-01-comprehensive-compaction-checkpoints.md)，而非简洁。
