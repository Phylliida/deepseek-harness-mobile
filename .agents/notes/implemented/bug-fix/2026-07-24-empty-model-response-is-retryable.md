...
**Treat whitespace-only or reasoning-only responses as empty too.** Rejected as overreach: those carry model-produced content, and misclassifying a legitimate (if useless) response as a transport-class failure risks retry loops on models that intentionally stop after reasoning. The scope is exactly "zero content blocks". The rejection holds unconditionally; [channel-leak markers](2026-08-15-channel-leak-markers-retryable-invisible-reply.md) later added the deployment-declared, marker-gated refinement for gateways that fold a real reply into the reasoning channel.
