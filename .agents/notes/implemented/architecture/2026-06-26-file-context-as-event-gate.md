...
- `fs/write-intent` listener: unseen/absent ⇒ `createIfAbsent`; present ⇒ `replaceIfVersion`. With `allowUnobservedMutation` configured, an unseen target instead falls through unguarded; observed-absent and observed-present decisions are unchanged. It does NOT call `next()`: it fully owns the single decision slot.
- `fs/edit-intent` listener: unseen ⇒ `FS_NOT_OBSERVED` (unguarded `undefined` under the same config); absent ⇒ `FS_NOT_FOUND`; present ⇒ its version guard. It does NOT call `next()`.
...
`dsh-fs-observation-policy` is now a pure policy/recording plugin with no service API — it influences the world only through the event gate. That is what removes the method coupling from `dsh-tool-fs`. The plugin's single `Config` field, `allowUnobservedMutation` (default `false`), relaxes only the unseen-target decisions above; its rationale and rejected alternatives live in [the unobserved-mutation-config Agent Note](../feature/2026-08-15-fs-policy-unobserved-mutation-config.md).
