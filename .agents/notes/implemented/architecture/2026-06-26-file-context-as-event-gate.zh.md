...
- `fs/write-intent` 监听器：未见/缺失 ⇒ `createIfAbsent`；存在 ⇒ `replaceIfVersion`。配置 `allowUnobservedMutation` 时，未见目标改为无防护落空；已观测为缺失和已观测为存在的决策不变。它不调用 `next()`：完全占据单一决策槽位。
- `fs/edit-intent` 监听器：未见 ⇒ `FS_NOT_OBSERVED`（同一配置下为无防护的 `undefined`）；缺失 ⇒ `FS_NOT_FOUND`；存在 ⇒ 返回其版本守卫。同样不调用 `next()`。
...
`dsh-fs-observation-policy` 现在是一个纯策略/记录插件，没有服务 API——它只通过事件门禁影响外界。这正是移除 `dsh-tool-fs` 方法耦合的关键。该插件唯一的 `Config` 字段 `allowUnobservedMutation`（默认 `false`）仅放宽上述未见目标的决策；其理由与被拒绝的替代方案见 [unobserved-mutation-config Agent Note](../feature/2026-08-15-fs-policy-unobserved-mutation-config.md)。
