...
后端 profile 共享模式约定但在必要的主机授权上有所不同。除部署方的 `devicePassthrough` 列表（仅 Linux 档实现——见[设备透传 Agent Note](2026-08-31-sandbox-device-passthrough.md)）外，Landlock 和 Seatbelt 在 read-only 模式下仅允许 `/dev/null`；workspace-write 还允许各自所需的主机临时目录根。每次包装携带后端特定的拒绝签名。Landlock 在较旧的 ABI 无法管控所有操作时报告 partial enforcement，而成功的 bwrap 和 Seatbelt profile 报告 full enforcement。
