...
flowchart LR
  ...
  pkg_scope["scope"]
  pkg_memory["memory"]
  svc_memory["ctx.memory<br/>Permanent agent-curated memory seam"]
...
| `ctx.memory` | `seam` | [`memory`](../packages/memory/memory) | [`memory-log`](../packages/memory/memory-log) | [`tool-memory`](../packages/memory/tool-memory) | - | 提供方负责仅追加记忆日志与摘要树的存储；工具 Consumer 渲染 wake/note/nap/recall/zoom/forget 对话。压缩由 agent 参与完成。 |
