...
来源：[`packages/coding/coding-activity/src/index.ts:44`](../packages/coding/coding-activity/src/index.ts)
...
export interface Config {
  /**
   * Allow writes and edits of targets this owner has never observed (default
   * `false`: a prior observation is required). `true` relaxes only the
   * UNSEEN-target decisions — an unseen write falls through to the bare
   * provider's unconditional mutation instead of `createIfAbsent`, and an
   * unseen edit proceeds without a version basis instead of rejecting with
   * `FS_NOT_OBSERVED`. Confirmed observations still decide: presence
   * CAS-guards at the observed version (`replaceIfVersion` / edit CAS, so
   * external changes since the observation still fail `FS_STALE_VERSION`),
   * and absence keeps `createIfAbsent` / `FS_NOT_FOUND`.
   */
  allowUnobservedMutation?: boolean
}
```

来源：[`packages/fs/fs-observation-policy/src/index.ts:21`](../packages/fs/fs-observation-policy/src/index.ts)
...
export interface PiAiModelProfile {
  ...
  compat?: PiAiCompatProfile
  /**
   * Channel-markup strings that must never appear in *reasoning* text. Some
   * gateways re-parse a model's channel grammar into thinking/text/tool blocks
   * and lose a boundary, folding the whole visible reply into one stale
   * thinking block and ending with a silent reasoning-only stop. When that
   * stop lands and this model's reasoning text contains any configured marker,
   * the adapter reports a retryable {@link CHANNEL_LEAK} error instead of
   * letting the turn end on invisible output.
   */
  channelLeakMarkers?: string[]
}
...
- `@deepseek-ai/dsh-client-ui-coding-timer`（[`packages/client/ui-coding-timer/src/index.ts`](../packages/client/ui-coding-timer/src/index.ts)）
- `@deepseek-ai/dsh-client-ui-commands`（[`packages/client/ui-commands/src/index.ts`](../packages/client/ui-commands/src/index.ts)）
- `@deepseek-ai/dsh-client-ui-conversation`（[`packages/client/ui-conversation/src/index.ts`](../packages/client/ui-conversation/src/index.ts)）
- `@deepseek-ai/dsh-client-ui-cordis`（[`packages/extensions/ui-cordis/src/index.ts`](../packages/extensions/ui-cordis/src/index.ts)）
- `@deepseek-ai/dsh-client-ui-cost-estimate`（[`packages/client/ui-cost-estimate/src/index.ts`](../packages/client/ui-cost-estimate/src/index.ts)）
...
- `@deepseek-ai/dsh-command-autobio` — 需要 `commands` · `compaction`（[`packages/compaction/command-autobio/src/index.ts`](../packages/compaction/command-autobio/src/index.ts)）
